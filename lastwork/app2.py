...
# 確認ボタン専用のCSVファイル名
CONFIRM_FILENAME = DATA_DIR + '/confirm_log.csv'

# 確認ボタン専用のCSVファイル名
CONFIRM_FILENAME = DATA_DIR + '/confirm_log.csv'

# ★【修正】確認ボタンが押されたときの処理（温度データ付き）
@app.route("/confirm", methods=["POST"])
def confirm():
    try:
        # JavaScriptから送られてきた温度データを取得
        tp = request.form.get('temperature', '-')
        
        # 現在の時刻を生成
        ts = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        
        # ログに書き込むデータ行 [時刻, ステータス, 温度]
        row = [ts, "確認済み", tp]
        
        # 確認用CSVファイルへ追記
        with open(CONFIRM_FILENAME, mode='a', encoding='utf-8', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(row)
            
        print(f"[HTTP Confirm] 確認ログ（温度付き）を保存しました: {row}")
        return "Success", 200
        
    except Exception as e:
        print(f"[HTTP Confirm Error] {e}")
        return "Internal Server Error", 500
